...
<B>第一句:</B><p>
<B>Serial.begin(115200);</B>是啟動Serial Communication並設定傳送速度(baud rate)為115200。這句包含.begin() functiony通常放在Void Setp()內。因為當Arduino UNO3一通電後便可以即時啟動<B>一次</B>。<p><B>Serial同begin()是關鍵字，不能改，大小寫要跟足。</B>begin()是function，已內嵌在Arduino IDE內並屬於Serial這個CLASS之下。所以Serial同begin()之間要有<B>"."</B>連接。<p> 
大家有沒有想過，當連接好硬件，如DHT11溫濕感應器後，怎樣利用軟件啟動? 在DHT11 Library，也有.begin()的function。所以通常.begin()放在Void Setp()內，當Arduino UNO3一通電後便可以即時啟動。
<p>  
<h1>應用例子</h1><p>
